...
// GitHub Pages SPA navigation: intercept internal links so clicks never request
// /airdrops, /wallet, etc. from the server (which would otherwise show GitHub 404).
document.addEventListener("click", (e)=>{
  const a=e.target.closest?.("a[href]");
  if(!a || e.defaultPrevented) return;
  if(a.target==="_blank" || a.hasAttribute("download") || e.ctrlKey || e.metaKey || e.shiftKey || e.altKey) return;
  ...
  if(path!==location.pathname+location.search+location.hash) go(path);
});
...
async function route(){
 let saved=sessionStorage.getItem("airdropx-route");if(saved && location.search.includes("airdropx_route=1")){sessionStorage.removeItem("airdropx-route");history.replaceState({},'',saved)}
