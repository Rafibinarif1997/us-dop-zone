-- AIRDROPX COMPLETE SUPABASE SCHEMA
-- Run this ONCE in Supabase SQL Editor.
-- Then create a user in Auth and insert their UUID into admin_users.
...
('Telegram Bots','telegram-bots','🤖',1),
('Testnets','testnets','⛓️',2),
('NFT Whitelist','nft-whitelist','🖼️',3),
('NFT GTD','nft-gtd','🎟️',4),
('Free Mint','free-mint','🆓',5),
('Other Web3','other-web3','🌐',6)
...
-- Atomic reward approval. Only admin can call it.
...
begin
  ...
  update public.profiles set available_balance=available_balance-p_amount
    where id=auth.uid() and available_balance>=p_amount;
  if not found then raise exception 'Insufficient balance'; end if;
  insert into public.withdrawals(user_id,amount,method,destination)
  values(auth.uid(),p_amount,p_method,p_destination) returning id into id_out;
  insert into public.wallet_transactions(user_id,type,amount,reference_id,description)
  values(auth.uid(),'withdrawal',-p_amount,id_out,'Withdrawal request');
  return id_out;
end; $$;
...
begin
  ...
  insert into public.wallet_transactions(user_id,type,amount,reference_id,description)
  values(w.user_id,'withdrawal_refund',w.amount,w.id,'Rejected withdrawal refund: '||coalesce(p_reason,'Rejected'));
end; $$;
...
create policy categories_public on public.categories for select using (status='active');

drop policy if exists campaigns_public on public.campaigns;
create policy campaigns_public on public.campaigns for select using (status='active');

drop policy if exists tasks_public on public.tasks;
create policy tasks_public on public.tasks for select using (status='active');
...
drop policy if exists submissions_self_select on public.task_submissions;
create policy submissions_self_select on public.task_submissions for select using (auth.uid()=user_id);
drop policy if exists submissions_self_insert on public.task_submissions;
create policy submissions_self_insert on public.task_submissions for insert with check (auth.uid()=user_id);

drop policy if exists transactions_self on public.wallet_transactions;
create policy transactions_self on public.wallet_transactions for select using (auth.uid()=user_id);

drop policy if exists withdrawals_self_select on public.withdrawals;
create policy withdrawals_self_select on public.withdrawals for select using (auth.uid()=user_id);
drop policy if exists withdrawals_self_insert on public.withdrawals;
create policy withdrawals_self_insert on public.withdrawals for insert with check (auth.uid()=user_id);
...
-- Admin read policies
create policy admin_campaigns on public.campaigns for all using (exists(select 1 from public.admin_users a where a.user_id=auth.uid())) with check (exists(select 1 from public.admin_users a where a.user_id=auth.uid()));
create policy admin_tasks on public.tasks for all using (exists(select 1 from public.admin_users a where a.user_id=auth.uid())) with check (exists(select 1 from public.admin_users a where a.user_id=auth.uid()));
create policy admin_submissions on public.task_submissions for select using (auth.uid()=user_id or exists(select 1 from public.admin_users a where a.user_id=auth.uid()));
create policy admin_submissions_update on public.task_submissions for update using (exists(select 1 from public.admin_users a where a.user_id=auth.uid()));
create policy admin_withdrawals on public.withdrawals for select using (auth.uid()=user_id or exists(select 1 from public.admin_users a where a.user_id=auth.uid()));
create policy admin_profiles on public.profiles for select using (auth.uid()=id or exists(select 1 from public.admin_users a where a.user_id=auth.uid()));
...
create policy proofs_read_auth on storage.objects for select to authenticated
